    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      ...
      >
        ...
        <div className="flex justify-center mt-8">
          <button
            disabled={loading}
            className="flex w-full justify-center cursor-pointer items-center gap-2 text-sm bg-gradient-to-r from-[#F6AB41] to-[#FF4938] text-white px-4 py-2 rounded-2xl "
          >
            {loading ? (
      ...
      <div className="w-full max-w-md p-4 bg-white rounded-lg border border-gray-200">
        ...
        >
          ...
          {processedImage ? (
            <img className=" w-full" src={processedImage} alt="" />
          ) : (
            <p className="text-gray-400 text-xs">
              {loading
                ? "This will take few seconds..."
                : 'Choose an image and click "Remove Background" to get started'}
            </p>
          )}
        </div>
      </div>
